(ns raytracer.grouping.hierarchy-test
...
(deftest test-to-world-coordinates
  (testing "Converting a point from world to object space"
    (let [sphere (shared/transform (shapes/sphere) (transform/translate 5 0 0))
          group2 (shared/transform (group/group [sphere]) (transform/scale 2 2 2))
          group1 (shared/transform (group/group [group2]) (transform/rotate-y const/half𝛑))
  ...
  (testing "Converting a normal from object to world space"
    (let [sphere (shared/transform (shapes/sphere) (transform/translate 5 0 0))
          group2 (shared/transform (group/group [sphere]) (transform/scale 1 2 3))
          group1 (shared/transform (group/group [group2]) (transform/rotate-y const/half𝛑))
          hierarchy (hierarchy/hierarchy group1)
  ...
  (testing "Converting a normal from object to world space: regression to check optimization"
    (let [sphere (shared/transform (shapes/sphere) (transform/translate 5 0 0))
          group4 (shared/transform (group/group [sphere]) (transform/scale 1 2 3))
          group3 (shared/transform (group/group [group4]) (transform/rotate-x 0.834))
          group2 (shared/transform (group/group [group3]) (transform/translate 3 2 1.2))
          group1 (shared/transform (group/group [group2]) (transform/rotate-y 0.23))
          hierarchy (hierarchy/hierarchy group1)
          third√3 (/ (Math/sqrt 3) 3)]
