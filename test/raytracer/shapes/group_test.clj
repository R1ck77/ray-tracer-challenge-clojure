...
(deftest test-local-intersect
  ...
  (testing "Intersection with a non-empty-group"
    (let [sphere1 (shapes/sphere)
          sphere2 (shared/transform (shapes/sphere) (transform/translate 0 0 -3))
          sphere3 (shared/transform (shapes/sphere) (transform/translate 5 0 0))
  ...
  (testing "Intersecting a transformed group (bounding box ignored)"
    (with-redefs [group/*use-bounding-boxes* false]
      (let [sphere (shared/transform (shapes/sphere)
                                     (transform/translate 5 0 0))
            group (shared/transform (group/group [sphere])
